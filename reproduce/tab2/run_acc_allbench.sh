...
case "$task" in
    perplexity)
        ...
        ;;
    zero-shot)
        basic_flags="--batch-size $batch --eval-zero-shot --eval-zero-shot-dataset arc_challenge,arc_easy,lambada_openai,piqa,winogrande,hellaswag"
        ;;
    zero-shot-hellaswag)
...
echo "TASK: INT4" | tee -a $logfilename
python3 ../../eval.py --model /home/dataset/$model $int4_flags $basic_flags 2>&1 | tee -a $logfilename
