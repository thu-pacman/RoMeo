...
struct SharedStorage {
    ...
    array_aligned<float, TileM> smemScaleA;
    array_aligned<float, TileN> smemScaleB;
};

template<int BS>
requires (BS == 256)
constexpr int SQRT() {
    if constexpr (BS == 256)  return 16;
}

template <int M, int N, int BS>
struct Array2DPartition {
    static constexpr int TX = SQRT<BS>();
    static constexpr int TY = BS / TX;

    static constexpr int num_x = M / TX;
    static constexpr int num_y = N / TY;

    int start_x, start_y;

    __host__ __device__ Array2DPartition(int tid) {
        CUTE_STATIC_ASSERT(M % TX == 0);
        CUTE_STATIC_ASSERT(N % TY == 0);
        start_x = (tid / TY) * num_x;
        start_y = (tid % TY) * num_y;
    }
};
...
__global__ void gemm_kernel(
    ...
    float *smemScaleA = smem.smemScaleA.data();
    float *smemScaleB = smem.smemScaleB.data();
    ...
    Tensor sScaleA = make_tensor(make_smem_ptr(smemScaleA), make_layout(make_shape(Int<TileM>{}))); // (TM)
    Tensor sScaleB = make_tensor(make_smem_ptr(smemScaleB), make_layout(make_shape(Int<TileN>{}))); // (TN)
    ...
    if (threadIdx.x < TileM) {
        sScaleA(threadIdx.x) = gScaleA_block(threadIdx.x);
    }
    if (threadIdx.x < TileN) {
        sScaleB(threadIdx.x) = gScaleB_block(threadIdx.x);
    }
    __syncthreads();
    ...
    Array2DPartition<WarpM, WarpN, size(mma)> part(threadIdx.x);
    CUTE_STATIC_ASSERT(part.num_y % 2 == 0);
        ...
        for (int j = 0; j < iter_n; j++) {
            ...
            for (int _2 = 0; _2 < size(get<2>(local_C.shape())); _2++) {
                for (int _1 = 0; _1 < size(get<1>(local_C.shape())); _1++) {
                    for (int _0 = 0; _0 < size(get<0>(local_C.shape())); _0 += 4) {
                        half c[4];
                        c[0] = local_C(_0, _1, _2, i, j);
                        c[1] = local_C(_0 + 1, _1, _2, i, j);
                        c[2] = local_C(_0 + 2, _1, _2, i, j);
                        c[3] = local_C(_0 + 3, _1, _2, i, j);
                        *reinterpret_cast<half2*>(&sC_thread(_0, _1, _2)) = *reinterpret_cast<half2*>(c);
                        *reinterpret_cast<half2*>(&sC_thread(_0 + 2, _1, _2)) = *reinterpret_cast<half2*>(c + 2);
                    }
                }
            }
            __syncthreads();
            // copy sC_int to sC, type convert, and scale
            for (int idx = 0; idx < part.num_x; idx++) {
                for (int idy = 0; idy < part.num_y; idy += 2) {
                    int x = part.start_x + idx, y = part.start_y + idy;
                    *reinterpret_cast<half2*>(&sC(x, y)) = __float22half2_rn(make_float2(
                        sC(x, y) * sScaleA(i * WarpM + x) * sScaleB(j * WarpN + y),
                        sC(x, y + 1) * sScaleA(i * WarpM + x) * sScaleB(j * WarpN + y + 1)
                    ));
                }
            }
            __syncthreads();
