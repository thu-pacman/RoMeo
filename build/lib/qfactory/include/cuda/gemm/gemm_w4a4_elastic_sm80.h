...
template <
    int N, int K, int Outlier, int Div,
...
__global__ void gemm_kernel(
    ...
    constexpr int NUM_GROUPS = 1 + Div;
    constexpr int GROUP_K = (K - Outlier) / Div;
    ...
    Tensor gScaleA = make_tensor(make_gmem_ptr<half_t>(A_scale), make_layout(make_shape(M, Int<NUM_GROUPS>{}), make_stride(Int<NUM_GROUPS>{}, _1{}))); // (M, NG)
    Tensor gScaleB = make_tensor(make_gmem_ptr<half_t>(B_scale), make_layout(make_shape(Int<N>{}, Int<NUM_GROUPS>{}), make_stride(Int<NUM_GROUPS>{}, _1{}))); // (N, NG)
    ...
    auto gScaleA_block = local_tile(gScaleA, make_tile(Int<TileM>{}, Int<NUM_GROUPS>{}), make_coord(idx, 0)); // (TM, NG)
    auto gScaleB_block = local_tile(gScaleB, make_tile(Int<TileN>{}, Int<NUM_GROUPS>{}), make_coord(idy, 0)); // (TN, NG)
    ...
    auto load_shared_scales = [&] (int load_k) {
        ...
        int next_k_idx = k_idx + TileK;
        if ((next_k_idx == Outlier) || (next_k_idx - Outlier) % GROUP_K == 0) { // fetch at the last iteration of each group
            int group_id = (k_idx < Outlier) ? 0 : (k_idx - Outlier) / GROUP_K + 1;
    ...
    for (int i = 0; i < Outlier / TileK; i++) {
        cp_async_wait<NStage - 1>();
        __syncthreads();
        do_wmma(i % NStage);
        load_block(i + NStage);
    }
    scale_acc(Outlier / TileK - 1);

    constexpr int niters = (K - Outlier) / TileK;
    constexpr int prefix_iters = Outlier / TileK;

    for (int i = 0; i < niters - NStage; i += acc_interval) {
        for (int j = 0; j < acc_interval && i + j < niters - NStage; j++) {
            cp_async_wait<NStage - 1>();
            __syncthreads();
            do_wmma((prefix_iters + i + j) % NStage);
            if (j == acc_interval - 1) {
                scale_acc(prefix_iters + i + j);
            }
            load_block(prefix_iters + i + j + NStage);
        }
    }
    ...
    auto pipe_tail = [&] <int i>() {
        cp_async_wait<i>();
        __syncthreads();
        do_wmma((prefix_iters + niters - 1 - i) % NStage);
        if ((niters - i) % acc_interval == 0) {
            scale_acc(prefix_iters + niters - 1 - i);
        }
    };
...
int call_gemm(
    ...
    assert(M % TileM == 0);
    assert(N % TileN == 0);
    assert(Outlier % TileK == 0);
    assert(((K - Outlier) / Div) % TileK == 0);
    assert((1 + Div) >= NStage - 1);
