...
template <typename smemALayout, typename smemBLayout, typename smemCLayout, typename smemTLayout, int TileM, int TileN>
struct SharedStorage {
    ...
    array_aligned<int8_t, cosize_v<smemTLayout>> smemT_8bit;
};

template <
    typename MMA_t, typename G2SCopyA_t, typename G2SCopyB_t, typename S2RCopyA_t, typename S2RCopyB_t, typename S2GCopyC_t,
    typename smemALayout_t, typename smemBLayout_t, typename smemCLayout_t, typename smemTLayout_t
>
struct KernelTraits {
    using MMA = MMA_t;
    ...
    using smemTLayout = smemTLayout_t;
};
...
__global__ void gemm_kernel(
    ...
    Tensor gB_8bit = make_tensor(make_gmem_ptr<int8_t>(B), make_layout(make_shape(Int<N>{}, Int<K>{}), make_stride(Int<K>{}, _1{}))); // (N, K)
    ...
    Tensor sB = make_tensor(make_smem_ptr<int8_t>(smem.smemB_8bit.data()), typename KTraits::smemBLayout{}); // (TN, TK, NStage)
    ...
    Tensor sTA = make_tensor(make_smem_ptr<int8_t>(smem.smemT_8bit.data()), typename KTraits::smemTLayout{}); // (TM, TK)
    ...
    auto sA_tile = local_tile(sTA, make_tile(Int<WarpM>{}, Int<WarpK>{}), make_coord(_, _)); // (WM, WK, iter_M, iter_K)
    ...
    Tensor local_A = mma_thread.partition_fragment_A(sA_tile(_, _, _, 0)); // (MMA, MMA_WM, MMA_WK, iter_M)
    ...
    auto sA_tile_s2r_thread = copyA_s2r_thread.partition_S(sA_tile); // (COPY, COPY_WM, COPY_WK, iter_M, iter_K)
    ...
    auto do_wmma = [&] (int k_idx) {
        ...
        for (int i = threadIdx.x; i < total_ops; i += size(mma)) {
            int im = i * 16 / TileK;
            int ik = i * 16 % TileK;
            uint64_t packed_int4s = *reinterpret_cast<uint64_t*>(raw_pointer_cast(&sA(im, ik, k_idx)));
            uint128_t int8s;
            decode_16x_int4_to_int8(packed_int4s, &int8s);
            *reinterpret_cast<uint128_t*>(&sTA(im, ik)) = int8s;
        }
        __syncthreads();
        for (int k = 0; k < iter_k; k++) {
            for (int i = 0; i < iter_m; i++) {
                copy(copyA_s2r, sA_tile_s2r_thread(_, _, _, i, k), local_A_s2r_view(_, _, _, i));
            }
...
int call_gemm(
    ...
    using sharedStorage = SharedStorage<smemALayout_4bit, smemBLayout_8bit, smemCLayout, smemALayout_8bit, TileM, TileN>;
    ...
    using G2SCopyB_8bit = decltype(make_tiled_copy(
    ...
    using KTraits = KernelTraits<
        MMA_8bit, G2SCopyA_4bit, G2SCopyB_8bit, S2RCopyA_8bit, S2RCopyB_8bit, S2GCopyC,
        smemALayout_4bit, smemBLayout_8bit, smemCLayout, smemALayout_8bit
    >;
