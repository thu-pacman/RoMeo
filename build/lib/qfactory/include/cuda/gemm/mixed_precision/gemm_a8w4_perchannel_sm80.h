...
__global__ void gemm_kernel(
    ...
    Tensor gA_8bit = make_tensor(make_gmem_ptr<int8_t>(A), make_layout(make_shape(M, Int<K>{}), make_stride(Int<K>{}, _1{}))); // (M, K)
    ...
    Tensor sA = make_tensor(make_smem_ptr<int8_t>(smem.smemA_8bit.data()), typename KTraits::smemALayout{}); // (TM, TK, NStage)
    ...
    Tensor sTB = make_tensor(make_smem_ptr<int8_t>(smem.smemT_8bit.data()), typename KTraits::smemTLayout{}); // (TN, TK)
    ...
    auto sB_tile = local_tile(sTB, make_tile(Int<WarpN>{}, Int<WarpK>{}), make_coord(_, _)); // (WN, WK, iter_N, iter_K)
    ...
    Tensor local_B = mma_thread.partition_fragment_B(sB_tile(_, _, _, 0)); // (MMA, MMA_WN, MMA_WK, iter_N)
    ...
    auto sB_tile_s2r_thread = copyB_s2r_thread.partition_S(sB_tile); // (COPY, COPY_WN, COPY_WK, iter_N, iter_K)
    ...
    auto do_wmma = [&] (int k_idx) {
        ...
        for (int i = threadIdx.x; i < total_ops; i += size(mma)) {
            int in = i * 16 / TileK;
            int ik = i * 16 % TileK;
            uint64_t packed_int4s = *reinterpret_cast<uint64_t*>(raw_pointer_cast(&sB(in, ik, k_idx)));
            uint128_t int8s;
            decode_16x_int4_to_int8(packed_int4s, &int8s);
            *reinterpret_cast<uint128_t*>(&sTB(in, ik)) = int8s;
        }
        __syncthreads();
        for (int k = 0; k < iter_k; k++) {
            ...
            for (int j = 0; j < iter_n; j++) {
                copy(copyB_s2r, sB_tile_s2r_thread(_, _, _, j, k), local_B_s2r_view(_, _, _, j));
            }
            for (int i = 0; i < iter_m; i++) {
                for (int j = 0; j < iter_n; j++) {
...
int call_gemm(
    ...
    using sharedStorage = SharedStorage<smemALayout_8bit, smemBLayout_4bit, smemCLayout, smemBLayout_8bit, TileM, TileN>;
    ...
    using KTraits = KernelTraits<
        MMA_8bit, G2SCopyA_8bit, G2SCopyB_4bit, S2RCopyA_8bit, S2RCopyB_8bit, S2GCopyC,
        smemALayout_8bit, smemBLayout_4bit, smemCLayout, smemBLayout_8bit
    >;
